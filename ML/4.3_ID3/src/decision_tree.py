...
def TreeGenerate(df):
    ...
    label_arr = df[df.columns[-1]] #get the last column "good watermelon" arrs
    #print("column-1: ", df.columns[-1])  #good watermelon
    #print("-------------------")
    #print("label_arr",label_arr)         #yes/no
    #print("-------------------")
...
def InfoGain(df, index):
    ...
    #print("df[index].dtypes:", df[index].dtypes)
    ...
    if df[index].dtype == (float, int):
        ...
        df = df.sort_index(ascending = True) #sort was deprecated for DataFrames 
                                             #in favor of needing to user either sort_values or sort_index
        #print("before reset:",df)
        df = df.reset_index(drop=True)  #When we reset the index, the old index is added as a column, 
                                        #and a new sequential index is used:
                                        #We can use the drop parameter to avoid the old index being added as a column:
        #print("after reset:", df)       #index:0 1 2 3 4 8 12 13 15 16
                                        #to 0-9
        data_arr = df[index]
